...
public class MyVis extends JPanel implements MouseListener, MouseMotionListener {
	...
	private DataType numericEnum;
	private DataType textualEnum;
	
	
	
	public MyVis() {
		super();
		message = "Parallel Lines Chart";
		nums = new ArrayList<>();
		...
		rangeX=(double) getWidth();
		rangeY= (double) getHeight();
	}
...
//		}
		//double maxY = Collections.max(nums);
		//double minY=Collections.min(nums);
		
//		double maxY=Collections.max(nums2);
//		double minY=Collections.min(nums2);
		numericEnum=DataType.NUMERIC;
		textualEnum=DataType.TEXTUAL;
		...
		g.draw(box);
		System.out.println("nums" + nums + " Labels " + labels);
	}
